...
float sga::evaluation(const permutation& p) {
	eval_count++;
	return wt_problem.evaluate(p);
}

void sga::evaluate_population(population& p) {
	for (unsigned int i = 0; i < p.size(); i++)
		p[i].eval = evaluation(p[i].perm);
}

population sga::initial_population() {
	population pop;
	for (int i = 0; i < population_size; ++i) {
		specimen s;
...
//bool sga::eval_comp(specimen a, specimen b) { return a.eval<b.eval; }

void sga::adapt_population(population& p) {
...
bool sga::optimum_termination(const population& p) {
	return p[0].eval <= cfg.optimum;
}

bool sga::termination(const population& p) {
	return optimum_termination(p) || (++iter > cfg.max_iter);
}

bool sga::smart_termination(const population& p) {
	return optimum_termination(p) || (deviate_count > population_size);
}

void sga::mutation_function(population& p) {
...
//ptrdiff_t sga::randgenid (ptrdiff_t i) { return randid()%i;}
//ptrdiff_t (*sga::p_randgenid)(ptrdiff_t) = randgenid;

void sga::crossover_function(population& p) {
...
void sga::replacement(population& p) {
...
void sga::report_end(population& p) {
