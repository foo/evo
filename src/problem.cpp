...
problem::problem() {
	...
	mutation_prob = 0.30f; // probability of mutation
...
void problem::init_prev_population() {
...
float problem::evaluation(const permutation& p) {
	eval_count++;
	return wt_problem.evaluate(p);
}

void problem::evaluate_population(population& p) {
...
//bool problem::eval_comp(specimen a, specimen b) { return a.eval<b.eval; }

void problem::adapt_population(population& p) {
	float F_min = min_element(p.begin(), p.end(),
			[](specimen a, specimen b) {return a.eval<b.eval;})->eval;
	for (unsigned int i = 0; i < p.size(); i++) {
		float sum = 0.0;
...
bool problem::optimum_termination(const population& p) {
	return p[0].eval <= cfg.optimum;
}

bool problem::termination(const population& p) {
	return optimum_termination(p) || (++iter > cfg.max_iter);
}

bool problem::smart_termination(const population& p) {
	return optimum_termination(p) || (deviate_count > population_size);
}

void problem::mutation_function(population& p) {
...
//ptrdiff_t problem::randgenid (ptrdiff_t i) { return randid()%i;}
//ptrdiff_t (*problem::p_randgenid)(ptrdiff_t) = randgenid;

void problem::crossover_function(population& p) {
	...
	// more randomness
	auto t = [](ptrdiff_t i) {return randid()%i;};
	random_shuffle(cross_set.begin(), cross_set.end(), t);
	...
	while (it != cross_set.end()) {
		...
		if (cfg.compare_operators) {
			...
			case 0:
				ctype = crossover::type::OX;
				break;
			case 1:
				ctype = crossover::type::CX;
				break;
			case 2:
			default:
				ctype = crossover::type::PMX;
				break;
			}
			desc = crossover::random_crossover(ctype, p[*it].perm,
					p[*(it + 1)].perm);
		} else {
			desc = crossover::random_crossover(cfg.crossover_type, p[*it].perm,
					p[*(it + 1)].perm);
		}
		...
		if (cfg.compare_operators
				&& (((p[*it].eval + p[*(it + 1)].eval) / (ch1.eval + ch2.eval))
						>= 1)) {
			switch (ctype) {
...
void problem::replacement(population& p) {
...
void problem::report(population& p) {
...
void problem::report_end(population& p) {
	if (cfg.report_every == config::report::none) {
		...
		if (cfg.report_best) {
			...
			std::cout << best_specimen.eval << " = " << best_specimen.perm
					<< std::endl;
		}
		...
		if (cfg.optimum > -1) {
			if (cfg.debug) {
				std::cout
						<< "Reporting number of evaluations needed to compute best specimen with given optimum value or --max-iter if optimum is not reached\n";
			}
			std::cout << population_size << ' ' << eval_count << "\n";
		}

		if (cfg.compare_operators) {
			std::cout << x_count << ' ' << ox_count << ' ' << cx_count << ' '
					<< pmx_count << "\n";
		}
	}
}
...
void problem::solve_flowshop(config& c) {
	cfg = c;
	...
	algorithm.termination =
			cfg.smart_termination ? smart_termination : termination;
